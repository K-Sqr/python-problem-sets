...
def sum_list(lst):
    #base case: list is empty or only one element
    #process first element, recurse on the rest
    #process the last element, recurse on the rest
    # output = 0 + sum_list(lst)
    #return output
    if not lst:
        return 0
    else:
...
# 	# Initialize a left pointer to the 0th index in the list
    # left = lst[0]
    left = 0
    # 	# Initialize a right pointer to the last index in the listr
    # right = lst[-1] 
    right = len(lst)-1
...
def sum_digits(n):
#U
# takes in an int as a param and returns the sum of all the values of it's digits
# for example 234 will return 9
# 

#plan
# if the int is len(n) = 1; return n
# recursive case go through the remaining numbers
