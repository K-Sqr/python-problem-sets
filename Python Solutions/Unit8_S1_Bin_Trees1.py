...
root: TreeNode = TreeNode(10, TreeNode(3), TreeNode(6))
...
bin_tree = TreeNode(10,TreeNode(10,None,None),TreeNode(10,None,None))
...
def inorder_traversal(root):
    values = []
    node = root
    inorder_traversal_help(node,values)
    return values
...
def size(root: TreeNode):
    return 0 if not root else 1 + size(root.left) + size(root.right)
